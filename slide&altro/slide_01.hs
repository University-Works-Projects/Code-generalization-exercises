-- ES 01
    ...
    -- Svolgimento mio
        gen op empty [] = empty
        gen op empty (x : l) = x op (gen l)     -- Primo tipo di sintatti
        gen op empty (x : l) = op ((gen l) x)   -- Secondo tipo di sintassi
        -- Nota: Le due sintassi sono equivalenti
